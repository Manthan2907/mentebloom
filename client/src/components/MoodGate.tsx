  ...
  return (
    <motion.main initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 h-dvh overflow-y-auto overscroll-contain bg-[#081019] p-3 text-[#f6f3eb] sm:p-6 md:grid md:place-items-center">
      <div className="relative mx-auto flex min-h-[calc(100vh-1.5rem)] w-full max-w-[1180px] flex-col overflow-hidden rounded-[2rem] border border-white/15 bg-[#15202b]/95 shadow-[0_40px_120px_rgba(0,0,0,0.48)] backdrop-blur-2xl sm:min-h-[calc(100vh-3rem)] md:h-[calc(100dvh-3rem)] md:min-h-0 md:grid md:grid-cols-[0.84fr_1.16fr]">
        ...
        <section className="relative flex min-h-0 min-w-0 flex-1 flex-col p-6 sm:p-10 md:p-8 lg:p-12">
          <div className="flex shrink-0 items-center justify-between pb-6">
            <div className="flex items-center gap-1.5" aria-label={`Step ${step === "mood" ? 1 : step === "context" ? 2 : 3} of 3`}>
              {["mood", "context", "reflection"].map((item, index) => <span key={item} className={`h-1.5 rounded-full transition-all ${step === item ? "w-8 bg-[#c9d8b5]" : index < ["mood", "context", "reflection"].indexOf(step) ? "w-3 bg-[#c9d8b5]/70" : "w-3 bg-white/15"}`} />)}
            </div>
          ...
          <AnimatePresence mode="wait">
            {step === "mood" && <motion.div key="mood" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.35 }} className="flex flex-col items-center text-center">
              ...
              <motion.div key={`face-${mood ?? "none"}`} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 300, damping: 18 }} className="relative z-20 mt-5 grid size-24 place-items-center rounded-full border border-white/20 text-5xl shadow-[inset_0_1px_0_rgba(255,255,255,0.35),0_20px_45px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:size-32" style={{ backgroundColor: mood ? `${selected.color}cc` : "rgba(255,255,255,0.08)" }}>
                <span role="img" aria-label={mood ? selected.name : "Neutral"}>{mood ? selected.emoji : "😐"}</span>
              </motion.div>

              <div className="relative mt-2 flex h-36 w-full max-w-md items-end justify-center sm:h-44 md:h-40 lg:h-44">
              ...
              <div className="sticky bottom-0 mt-8 flex w-full max-w-md flex-col items-center gap-4 bg-[#15202b] py-4 sm:flex-row sm:justify-between"><p className="text-xs text-white/35">There is no better or worse answer. Noticing is enough.</p><button type="button" disabled={!mood} onClick={() => setStep("context")} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#c9d8b5] px-5 py-2.5 text-sm font-semibold text-[#18212b] transition hover:bg-[#d9e8c6] disabled:cursor-not-allowed disabled:opacity-35">Continue <ArrowRight className="size-4" /></button></div>
            </motion.div>}

            {step === "context" && <motion.div key="context" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -18 }}><div className="mb-8 flex items-center gap-3"><span className="size-3 rounded-full" style={{ backgroundColor: selected.color }} /><p className="text-sm text-white/55">You feel <span className="font-semibold text-white">{selected.name.toLowerCase()}</span> today.</p></div><h2 className="max-w-lg font-display text-3xl leading-tight sm:text-5xl">What is part of the picture?</h2><div className="mt-8 flex flex-wrap gap-3">{FACTORS.map((factor) => <button type="button" key={factor} onClick={() => setFactors((current) => current.includes(factor) ? current.filter((item) => item !== factor) : [...current, factor])} className={`rounded-full border px-5 py-3 text-sm transition-colors ${factors.includes(factor) ? "border-[#c9d8b5] bg-[#c9d8b5] text-[#18212b]" : "border-white/15 text-white/65 hover:border-white/40"}`}>{factors.includes(factor) && <Check className="mr-2 inline size-4" />}{factor}</button>)}</div><div className="sticky bottom-0 mt-8 bg-[#15202b] py-4"><button type="button" onClick={() => setStep("reflection")} className="inline-flex items-center gap-3 rounded-full bg-[#c9d8b5] px-6 py-3 text-sm font-semibold text-[#18212b]">Continue <ArrowRight className="size-4" /></button></div></motion.div>}

            {step === "reflection" && <motion.div key="reflection" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -18 }}><div className="mb-8 flex items-center gap-3"><PenLine className="size-5 text-[#c9d8b5]" /><p className="text-sm text-white/55">Optional, but useful for your future self.</p></div><h2 className="max-w-lg font-display text-3xl leading-tight sm:text-5xl">Give the feeling a little context.</h2><textarea autoFocus value={note} onChange={(event) => setNote(event.target.value)} maxLength={280} rows={5} placeholder="What is on your mind?" className="mt-8 w-full resize-none rounded-2xl border border-white/15 bg-white/[0.05] p-4 text-base leading-7 text-white outline-none placeholder:text-white/25 focus:border-[#c9d8b5]" /><div className="mt-2 flex items-center justify-between text-xs text-white/30"><span>Your words stay in your journal.</span><span>{note.length}/280</span></div><div className="sticky bottom-0 mt-8 bg-[#15202b] py-4"><button type="button" onClick={finish} className="inline-flex items-center gap-3 rounded-full bg-[#c9d8b5] px-6 py-3 text-sm font-semibold text-[#18212b]">See my thought <Sparkles className="size-4" /></button></div></motion.div>}

            {step === "ready" && <motion.div key="ready" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}><Sparkles className="mb-8 size-8 text-[#c9d8b5]" /><p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-[#c9d8b5]/70">Your thought for today</p><h2 className="max-w-xl font-display text-3xl leading-tight sm:text-5xl">{insight}</h2><p className="mt-8 text-sm leading-6 text-white/45">We will carry this gently into your dashboard.</p><div className="sticky bottom-0 mt-8 bg-[#15202b] py-4"><button type="button" onClick={onComplete} className="inline-flex items-center gap-3 rounded-full border border-white/20 px-6 py-3 text-sm text-white hover:border-[#c9d8b5]">Open dashboard <ArrowRight className="size-4" /></button></div></motion.div>}
          </AnimatePresence>
          </div>
          {step !== "ready" && <div className="flex shrink-0 items-center justify-between border-t border-white/10 bg-[#15202b] pt-4 text-xs text-white/25">{step === "mood" ? "01 / 03" : step === "context" ? "02 / 03" : "03 / 03"}</div>}
        </section>
      </div>
    </motion.main>
  );
